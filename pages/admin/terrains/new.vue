...
function onDrop(i) {
  ...
  const img = images.value.splice(dragIndex.value, 1)[0]
  images.value.splice(i, 0, img)
  dragIndex.value = -1
}
...
function handleMediaDrop(e) {
  ...
  if (docs.length) {
    for (const file of docs) {
      documents.value.push({ file, type: 'attestation_villageoise', label: file.name })
    }
  }
}
...
async function handleSubmit() {
  ...
  saving.value = true
  try {
    const terrain = await $api('/api/admin/terrains', {
      method: 'POST',
      body: {
        ...form.value,
    ...
    show('Erreur lors de la création', 'error')
...
<template>
  <div class="max-w-4xl mx-auto px-5 md:px-8 py-6">
    <div class="flex items-center justify-between mb-7">
      <div>
        <h1 class="text-2xl font-heading font-bold text-navy">Nouveau terrain</h1>
        <p class="text-sm text-gray-500 mt-0.5">Ajoutez un terrain au catalogue</p>
      </div>
    ...
    <form @submit.prevent="handleSubmit" class="space-y-6">
      <div class="bg-white rounded-2xl border border-gray-100 p-6 md:p-7 shadow-sm">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-4">
          <div>
            ...
            <input v-model="form.lieu" required class="w-full px-3.5 py-2.5 bg-gray-50 border border-gray-200 rounded-xl focus:bg-white focus:ring-2 focus:ring-navy/10 focus:border-navy outline-none transition-all text-sm placeholder:text-gray-400" placeholder="Bonoua" />
          </div>
          <div>
            <label class="block text-sm font-medium text-gray-600 mb-1.5">Quartier <span class="text-red-400">*</span></label>
            <input v-model="form.quartier" required class="w-full px-3.5 py-2.5 bg-gray-50 border border-gray-200 rounded-xl focus:bg-white focus:ring-2 focus:ring-navy/10 focus:border-navy outline-none transition-all text-sm placeholder:text-gray-400" placeholder="Yaou Nouveau Quartier" />
          </div>
          <div>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-4">
          <div>
            ...
            <div class="flex gap-2">
              <input v-model="superficieValeur" type="number" step="any" min="0" required class="w-full px-3.5 py-2.5 bg-gray-50 border border-gray-200 rounded-xl focus:bg-white focus:ring-2 focus:ring-navy/10 focus:border-navy outline-none transition-all text-sm placeholder:text-gray-400" placeholder="600" />
          ...
          <div>
            ...
            <input v-model="form.prix" required class="w-full px-3.5 py-2.5 bg-gray-50 border border-gray-200 rounded-xl focus:bg-white focus:ring-2 focus:ring-navy/10 focus:border-navy outline-none transition-all text-sm placeholder:text-gray-400" placeholder="3 500 000 FCFA" />
          </div>
          <div>
        ...
        <div>
          ...
          <textarea v-model="form.description" rows="4" class="w-full px-3.5 py-2.5 bg-gray-50 border border-gray-200 rounded-xl focus:bg-white focus:ring-2 focus:ring-navy/10 focus:border-navy outline-none transition-all text-sm placeholder:text-gray-400 resize-none" placeholder="Description détaillée du terrain..."></textarea>
        </div>
      </div>
      ...
      <div class="bg-white rounded-2xl border border-gray-100 p-6 md:p-7 shadow-sm">
        ...
        <div
          ...
          @click="document.getElementById('media-input')?.click()"
        ...
        >
          ...
          <input id="media-input" type="file" accept="image/*,video/mp4,video/webm,video/quicktime" multiple class="hidden" @change="onMediaSelect" :disabled="uploading" />
        </div>
        ...
        <div v-if="images.length" class="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-5 gap-3 mt-5">
          <div
          ...
          >
            ...
            <button type="button" @click="removeImage(i)" class="absolute top-1.5 right-1.5 w-6 h-6 bg-white/90 text-red-500 rounded-full text-xs opacity-0 group-hover:opacity-100 transition-all flex items-center justify-center hover:bg-red-500 hover:text-white shadow-sm z-10">&times;</button>
          </div>
        </div>
      </div>
      ...
      <div class="bg-white rounded-2xl border border-gray-100 p-6 md:p-7 shadow-sm">
        ...
        <div
          ...
          @click="document.getElementById('doc-input')?.click()"
        ...
        >
          ...
          <input id="doc-input" type="file" accept=".pdf,.doc,.docx,.jpg,.jpeg,.png" multiple class="hidden" @change="onDocumentSelect" />
        </div>
        ...
        <div v-if="documents.length" class="space-y-2 mt-5">
          <p class="text-xs font-medium text-gray-400 uppercase tracking-wider">Fichiers sélectionnés</p>
          <div v-for="(doc, i) in documents" :key="i" class="flex items-center gap-3 bg-gray-50 rounded-xl px-4 py-3 border border-gray-100 group hover:border-gray-200 transition-all">
            ...
            <button type="button" @click="removeDocument(i)" class="p-1.5 text-gray-400 hover:text-red-500 hover:bg-red-50 rounded-lg transition-all shrink-0">
      ...
      <div v-if="error" class="bg-red-50 border border-red-100 text-red-600 text-sm rounded-xl px-5 py-3.5 flex items-center gap-2">
